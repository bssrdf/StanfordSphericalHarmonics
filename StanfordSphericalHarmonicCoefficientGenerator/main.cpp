//
//  main.cpp
//  StanfordSphericalHarmonicCoefficientGenerator
//
//  Created by turner on 3/10/15.
//  Copyright (c) 2015 Elastic Image Software. All rights reserved.
//

/******************************************************************************

Program to prefilter environment maps, computing spherical harmonic lighting coefficients.

Input
-----
Name of file storing high-dynamic range environment.
...
Output
------
RGB values for lighting coefficients L_{lm} with 0 <= l <= 2 and -l <= m <= l.
There are a total of 9 coefficients.

Reference
---------
This is an implementation of the method described by
Ravi Ramamoorthi and Pat Hanrahan in their SIGGRAPH 2001 paper,
...
% StanfordSphericalHarmonicCoefficientGenerator -swap grace_probe.float 1000
% StanfordSphericalHarmonicCoefficientGenerator -swap rnl_probe.float 900
% StanfordSphericalHarmonicCoefficientGenerator -swap stpeters_probe.float 1500
    ...
    for (i = 0 ; i < width ; i++)
        for (j = 0 ; j < width ; j++)
            for (k = 0 ; k < 3 ; k++) {
                ...
                /* This is a little ugly since input is binary and we need to test endian-ness */
                float val ;
...
void prefilter(int width) {
    ...
    for (int i = 0 ; i < width ; i++)
        for (int j = 0 ; j < width ; j++) {
            ...
            v = (float) ((width/2.0 - i)/(width/2.0));  /* v ranges from -1 to 1 */
            u = (float) ((j-width/2.0)/(width/2.0));    /* u ranges from -1 to 1 */
            r = (float) sqrt(u*u+v*v);               /* The "radius" */
            if (r > 1.0) continue ;           /* Consider only circle with r<1 */

            theta = (float) (PI*r);                    /* theta parameter of (i,j) */
            phi = atan2f(v,u) ;                /* phi parameter */

            x = sinf(theta)*cosf(phi) ;         /* Cartesian components */
           ...
           if the form of the input changes */
            ...
            domega = (float) ((2*PI/width)*(2*PI/width)*sinc(theta));
